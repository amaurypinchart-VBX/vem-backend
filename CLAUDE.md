...
- `plans/src/sheets/` + `plans/src/ui/sheets/` — drawing sets (P3): document model (`types.ts`: sheets of items in paper mm; viewports store a `LineworkRequest`, never linework), Viewbox A1 template measured on the reference LayOut PDFs (`template.ts`; A3 = same template scaled), vector logos traced from those PDFs (`logos.ts`), SVG renderer used for screen/thumbnails/export (`SheetSvg.tsx`), generator reproducing the reference sets (`generate.ts`: cover, 4 views, long/short sides, implantation, per-Viewbox sheet…), Zustand + Immer patch history (`store.ts`), in-memory linework per viewport + stale detection (`bank.ts`). Editor autosaves 2 s after the last change to `plans_drawing_sets` (`data` JSON). 3D images on sheets are rendered by an offscreen `SceneViewer` (`viewer/offscreenCapture.ts`; viewers share `LoadedScene.root`, `reclaim()` re-attaches it) and uploaded as Cloudinary images (`POST /plans/project/:id/assets`).
- Dimensions (P4): `DimensionItem` stores 3D anchors + the viewport id (associative: projected through the viewport's view basis, so they follow scale/framing/recalculation); `sheets/dimensions.ts` = pure geometry (architect style §3.5: 2.5 mm Gelasio text, 45° ticks, 2 mm extension gap/overshoot) + auto layout (`planDimensions`: chains + totals measured on Viewbox frames, not protruding feet; rows 10 mm then +7 mm; `fitWithDims` picks the scale that leaves room for them); `sheets/autoDim.ts` reads doors (PORTE-*), feet (PIED), roof band (TOIT) from the view's `sourceNodeIds`; `sheets/snap.ts` = endpoint/midpoint/circle-centre snapping for the manual Cote / Chaîne tools.
- Browser tests: `plans/e2e/` (dev only, not deployed) — mock VEM backend `server.mjs` + Playwright scripts (`run-sheets.mjs`, `export-svg.mjs`); fixture from `npm run e2e:fixture`; screenshots in `plans/e2e/shots/` (gitignored). Needs the Chromium in `~/.cache/ms-playwright` (or `CHROMIUM=`).
...
- `app_settings` keys `plans.*` (classification rules) can be written by admin, technical_manager and engineer (`src/routes/settings.ts`); other keys stay admin-only.
