...
exports.sendTicketAssigned = sendTicketAssigned;
exports.sendDailyReport = sendDailyReport;
exports.sendHandoverPdf = sendHandoverPdf;
exports.sendTicketEscalation = sendTicketEscalation;
...
async function sendViaBrevo(opts) {
    ...
    if (!r.ok) {
        logger_1.logger.error(`[email/brevo] HTTP ${r.status} : ${txt}`);
    ...
    catch (_) { /* ok pour 204 No Content */ }
...
async function sendViaAppsScript(opts) {
    const url = process.env.GMAIL_APPS_SCRIPT_URL;
    const token = process.env.GMAIL_APPS_SCRIPT_TOKEN;
    if (!url || !token)
        throw new Error('GMAIL_APPS_SCRIPT_URL ou GMAIL_APPS_SCRIPT_TOKEN manquant');
    const body = {
        token,
        to: opts.to,
        subject: opts.subject,
    ...
    /[\u007F-\uFFFF]/g, (c) => '\\u' + ('0000' + c.charCodeAt(0).toString(16)).slice(-4));
    ...
    while ((r.status === 301 || r.status === 302 || r.status === 303 || r.status === 307 || r.status === 308) && redirects < 5) {
        ...
        if (!location)
            break;
        redirects++;
        logger_1.logger.info(`[email/apps-script] Redirect ${r.status} → ${location.slice(0, 80)}...`);
    ...
    const txt = await r.text();
    if (!r.ok) {
        logger_1.logger.error(`[email/apps-script] HTTP ${r.status} : ${txt.slice(0, 300)}`);
...
async function sendMail(opts) {
    ...
    if (process.env.GMAIL_APPS_SCRIPT_URL && process.env.GMAIL_APPS_SCRIPT_TOKEN) {
        try {
            ...
            logger_1.logger.info(`[email/apps-script] ✅ envoyé à ${toStr} (via Gmail)`);
            return result;
        }
        catch (e) {
            logger_1.logger.error(`[email/apps-script] ❌ échec : ${e.message}`);
    ...
    if (provider === 'brevo') {
        ...
        try {
            ...
            const toStr = Array.isArray(opts.to) ? opts.to.join(', ') : opts.to;
            logger_1.logger.info(`[email/brevo] ✅ envoyé à ${toStr} (messageId=${result.messageId})`);
            return result;
        }
        catch (brevoErr) {
            logger_1.logger.error(`[email/brevo] ❌ échec : ${brevoErr.message}`);
    ...
    try {
        logger_1.logger.info(`[email/smtp] tentative d'envoi via ${process.env.SMTP_HOST || 'smtp.gmail.com'}:${PORT} (user=${process.env.SMTP_USER})`);
        ...
        const toStr = Array.isArray(opts.to) ? opts.to.join(', ') : opts.to;
        logger_1.logger.info(`[email/smtp] ✅ envoyé à ${toStr} (messageId=${result.messageId})`);
        return result;
    }
    catch (err) {
        ...
        logger_1.logger.error(`[email/smtp] ❌ échec : ${msg}${hint}`);
...
async function sendTicketAssigned(opts) {
...
async function sendHandoverPdf(opts) {
...
async function sendTicketEscalation(opts) {
    const content = `
