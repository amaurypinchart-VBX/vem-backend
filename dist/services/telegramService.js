...
exports.tubizeRoleForTruck = tubizeRoleForTruck;
exports.notifyTubizeTruckMovement = notifyTubizeTruckMovement;
...
async function sendTelegramMessage(text) {
    ...
    if (!token || !chatId) {
        logger_1.logger.warn('[telegram] TELEGRAM_BOT_TOKEN ou TELEGRAM_WAREHOUSE_CHAT_ID non configuré — message ignoré');
        return false;
    }
    try {
        ...
        if (!r.ok) {
            ...
            logger_1.logger.error(`[telegram] API a répondu ${r.status}: ${err.slice(0, 300)}`);
            return false;
        }
        logger_1.logger.info('[telegram] Message envoyé OK');
        return true;
    }
    catch (e) {
        logger_1.logger.error(`[telegram] Échec envoi : ${e.message || e}`);
        return false;
    }
}
/**
 ...
 */
async function notifyTubizeTruckMovement(truck, project, action = 'created') {
    const role = tubizeRoleForTruck(truck);
    if (!role)
        return false; // Pas concerné par Tubize → on ignore
    ...
    let dateValue = truck.loadingDate;
    if (role === 'arrival') {
    ...
    if (truck.driverName)
        lines.push(`🚚 <b>Chauffeur :</b> ${escapeHtml(truck.driverName)}${truck.driverPhone ? ' — ' + escapeHtml(truck.driverPhone) : ''}`);
    lines.push(`👤 <b>À assigner :</b> ${escapeHtml(assignee)}`);
    if (truck.notes)
        lines.push(`📝 <i>${escapeHtml(truck.notes)}</i>`);
