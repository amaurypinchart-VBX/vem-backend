...
exports.gatherProjectReportData = gatherProjectReportData;
...
async function gatherProjectReportData(projectId) {
    ...
    const [tasksTotal, tasksDone, tasksOverdue, taskList] = await Promise.all([
        database_1.prisma.task.count({ where: { projectId } }),
        database_1.prisma.task.count({ where: { projectId, status: 'done' } }),
        database_1.prisma.task.count({ where: { projectId, status: { notIn: ['done', 'cancelled'] }, taskDate: { lt: new Date() } } }),
    ...
    const [ticketsTotal, ticketsOpen, ticketsCritical, ticketList] = await Promise.all([
        database_1.prisma.ticket.count({ where: { projectId } }),
        database_1.prisma.ticket.count({ where: { projectId, status: { in: ['open', 'assigned', 'in_progress'] } } }),
        database_1.prisma.ticket.count({ where: { projectId, urgency: 'critical', status: { not: 'closed' } } }),
    ...
    const dailyReports = await database_1.prisma.dailyReport.findMany({
...
function buildStructuredSections(data) {
    ...
    if (p.dismantlingStart)
        lines.push(`Démontage : ${fmtDate(p.dismantlingStart)} → ${fmtDate(p.dismantlingEnd)}`);
    ...
    if (data.team.length === 0)
        lines.push('Aucun membre assigné.');
    for (const m of data.team)
        lines.push(`- ${m.user.firstName} ${m.user.lastName} — ${m.role}${m.isLead ? ' (lead)' : ''}`);
    lines.push('');
...
async function synthesizeNarrative(data) {
    const p = data.project;
...
Règles impératives : n'invente JAMAIS un fait, un nom ou un chiffre absent des données ci-dessus. Si une information manque pour juger d'un point, dis-le simplement plutôt que de l'inventer.`;
    return (0, aiService_1.callClaude)({ maxTokens: 900, messages: [{ role: 'user', content: prompt }] });
}
...
//# sourceMappingURL=projectReportService.js.map