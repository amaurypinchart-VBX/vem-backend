...
exports.createWarehouseTask = createWarehouseTask;
 ...
 */
async function createWarehouseTask(truck, project) {
    const role = (0, telegramService_1.tubizeRoleForTruck)(truck);
    if (!role)
        return false; // Pas concerné par Tubize
    const { supabaseUrl, supabaseKey, assigneeId, creatorId } = getConfig();
    if (!supabaseUrl || !supabaseKey || !creatorId) {
        logger_1.logger.warn('[warehouseApp] Config Supabase incomplète (URL/KEY/CREATOR_ID) — task ignorée');
        return false;
    }
    ...
    if (truck.truckNumber)
        descLines.push(`N° camion : ${truck.truckNumber}`);
    if (truck.licensePlate)
        descLines.push(`Plaque : ${truck.licensePlate}`);
    if (truck.driverName) {
        descLines.push(`Chauffeur : ${truck.driverName}${truck.driverPhone ? ' (' + truck.driverPhone + ')' : ''}`);
    }
    if (truck.notes)
        descLines.push('Notes : ' + truck.notes);
    ...
    try {
        ...
        if (!r.ok) {
            ...
            logger_1.logger.error(`[warehouseApp] Supabase a répondu ${r.status}: ${errText.slice(0, 300)}`);
            return false;
        }
        logger_1.logger.info(`[warehouseApp] Task créée dans EntrepôtApp : "${title}"`);
        return true;
    }
    catch (e) {
        logger_1.logger.error(`[warehouseApp] Échec création task : ${e.message || e}`);
        return false;
    }
}
//# sourceMappingURL=warehouseAppService.js.map