...
async function seedTaskTemplatesIfEmpty() {
    try {
        ...
        const tplCount = await database_1.prisma.taskTemplate.count();
        if (cats.length === 0) {
            // Cas 1 : tout vide → seed complet
            logger_1.logger.info('[seed] task templates : base vide, import complet...');
            for (let i = 0; i < TASK_TEMPLATES_SEED.length; i++) {
                ...
                logger_1.logger.info(`[seed]   → ${cat.name} (${cat.tasks.length} tâches)`);
            }
            logger_1.logger.info('[seed] ✅ Templates Viewbox importés en base');
            return;
        }
        if (tplCount === 0) {
            ...
            logger_1.logger.info(`[seed] task templates : ${cats.length} catégorie(s) OK mais 0 templates, re-seed des templates...`);
            let totalCreated = 0;
            const catsByName = {};
            cats.forEach((c) => { catsByName[c.name] = c; });
            for (let i = 0; i < TASK_TEMPLATES_SEED.length; i++) {
                const seedCat = TASK_TEMPLATES_SEED[i];
                let cat = catsByName[seedCat.name];
                if (!cat) {
                    // Catégorie pas trouvée → on la crée
                    cat = await database_1.prisma.taskCategory.create({
            ...
            logger_1.logger.info(`[seed] ✅ ${totalCreated} template(s) recréé(s) sur les catégories existantes`);
            return;
        }
        logger_1.logger.info(`[seed] task templates : ${cats.length} catégorie(s) et ${tplCount} template(s) en base, seed sauté`);
    }
    catch (err) {
        logger_1.logger.error(`[seed] échec d'import des templates : ${err.message || err}`);
    }
}
async function runStartupMigrations() {
    ...
    `);
        ...
        for (const col of [
            ...
            await database_1.prisma.$executeRawUnsafe(`ALTER TABLE "users" ADD COLUMN IF NOT EXISTS "${col}" TEXT;`);
        }
        ...
        for (const col of ['birth_date', 'id_expiry']) {
            try {
                await database_1.prisma.$executeRawUnsafe(`ALTER TABLE "users" ALTER COLUMN "${col}" TYPE TEXT USING "${col}"::text;`);
                logger_1.logger.info(`[migration] users.${col} forcé en TEXT`);
            }
            catch (e) {
                logger_1.logger.warn(`[migration] conversion ${col} : ${e.message}`);
            }
        }
        ...
        for (const val of ['sales_engineer', 'installer']) {
            try {
                await database_1.prisma.$executeRawUnsafe(`ALTER TYPE "UserRole" ADD VALUE IF NOT EXISTS '${val}';`);
            }
            catch (e) {
                logger_1.logger.warn(`[migration] enum value "${val}" : ${e.message}`);
            }
        }
    ...
    `);
        ...
        for (const val of ['quote_to_validate', 'quote_validated', 'handover_ok']) {
            try {
                await database_1.prisma.$executeRawUnsafe(`ALTER TYPE "ProjectStatus" ADD VALUE IF NOT EXISTS '${val}';`);
            }
            catch (e) {
                logger_1.logger.warn(`[migration] ProjectStatus value "${val}" : ${e.message}`);
            }
        }
  ...
  `);
        logger_1.logger.info('[migration] handovers.custom_fields ajoutée si absente');
        ...
        try {
            await database_1.prisma.$executeRawUnsafe(`ALTER TABLE "daily_report_entries" ALTER COLUMN "entry_time" DROP NOT NULL;`);
        }
        catch (e) {
            logger_1.logger.warn(`[migration] daily_report_entries.entry_time : ${e.message}`);
        }
    ...
    `);
        logger_1.logger.info('[migration] task_templates.categoryId ajoutée si absente + orphelins purgés');
      ...
      `);
            logger_1.logger.info('[migration] task_templates.category_id (colonne orpheline) supprimée');
        }
        catch (e) {
            // Si DROP échoue (FK ou autre), on tente au moins de retirer le NOT NULL
            logger_1.logger.warn(`[migration] DROP category_id échoué (${e.message}), tentative DROP NOT NULL...`);
            try {
                await database_1.prisma.$executeRawUnsafe(`
          ALTER TABLE "task_templates" ALTER COLUMN "category_id" DROP NOT NULL;
        `);
                logger_1.logger.info('[migration] task_templates.category_id : NOT NULL retiré');
            }
            catch (e2) {
                logger_1.logger.warn(`[migration] DROP NOT NULL category_id : ${e2.message}`);
            }
        }
    ...
    `);
        ...
        END $$;
      `);
            logger_1.logger.info('[migration] contrainte unique daily_reports (project_id, report_date) supprimée si présente');
        }
        catch (e) {
            logger_1.logger.warn(`[migration] drop unique daily_reports : ${e.message}`);
        }
    ...
    `);
        logger_1.logger.info('[migration] projects : scope / install_notes / dismantle_notes ajoutées si absentes');
    ...
    `);
        logger_1.logger.info('[migration] task_categories / task_templates : createdAt/updatedAt ajoutées si absentes');
    ...
    `);
        logger_1.logger.info('[migration] team_bookings / hotel_bookings : colonnes attachment ajoutées si absentes');
    ...
    `);
        logger_1.logger.info('[migration] project_team.phase ajoutée si absente');
    ...
    `);
        logger_1.logger.info('[migration] projects.sort_order ajoutée si absente');
        // ─── handovers.scope_of_work : étendue des travaux éditable ───
        await database_1.prisma.$executeRawUnsafe(`
    ALTER TABLE handovers ADD COLUMN IF NOT EXISTS scope_of_work TEXT;
  `);
        logger_1.logger.info('[migration] handovers.scope_of_work ajoutée si absente');
  ...
  `);
        logger_1.logger.info('[migration] handovers : colonnes signature_token ajoutées si absentes');
    ...
    `);
      ...
      `);
        ...
        logger_1.logger.info('[migration] clients.vat + table client_contacts créées si absentes');
    ...
    `);
        logger_1.logger.info('[migration] table app_settings créée si absente');
        logger_1.logger.info('✅ Migrations de démarrage OK');
        ...
        try {
            const cols = await database_1.prisma.$queryRawUnsafe(`SELECT column_name, data_type FROM information_schema.columns WHERE table_name = 'users' ORDER BY ordinal_position;`);
            logger_1.logger.info(`[diag] colonnes users : ${cols.map(c => `${c.column_name}=${c.data_type}`).join(', ')}`);
            ...
            logger_1.logger.info(`[diag] UserRole enum : ${enumVals.map(e => e.enumlabel).join(', ')}`);
        }
        catch (e) {
            logger_1.logger.warn(`[diag] échec lecture méta : ${e.message}`);
        }
        ...
        try {
            ...
            logger_1.logger.info('🔄 Prisma reconnecté pour rafraîchir les métadonnées');
        }
        catch (e) {
            logger_1.logger.warn(`[migration] reconnexion Prisma échouée : ${e.message}`);
        }
        ...
        logger_1.logger.error('❌ Erreur lors des migrations de démarrage :', err);
