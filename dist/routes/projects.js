    ...
    try {
        ...
        const enriched = await Promise.all(projects.map(async (p) => {
            const [total, done] = await Promise.all([
                database_1.prisma.task.count({ where: { projectId: p.id } }),
                database_1.prisma.task.count({ where: { projectId: p.id, status: 'done' } }),
            ]);
...
router.patch('/reorder', async (req, res, next) => {
    try {
        ...
        await database_1.prisma.$transaction(orders.map((o) => database_1.prisma.project.update({
            where: { id: o.id },
...
router.get('/:id', async (req, res, next) => {
    try {
        const p = await database_1.prisma.project.findUnique({
    ...
    try {
        ...
        const project = await database_1.prisma.project.create({
            data: {
                ...
                team: {
                    create: [
                        ...siteManagerIds.map((uid) => ({ userId: uid, role: 'site_manager', isLead: true })),
                        ...engineerIds.map((uid) => ({ userId: uid, role: 'engineer' })),
                    ],
                },
            },
            include: { client: true },
...
router.patch('/:id', async (req, res, next) => {
    try {
        ...
        index_1.io.to(`project:${req.params.id}`).emit('project:updated', project);
...
router.post('/:id/team', async (req, res, next) => {
    try {
        ...
        if (err.code === 'P2002')
            return res.status(409).json({ success: false, error: 'Déjà dans l\'équipe' });
        next(err);
    }
});
...
router.patch('/:id/team/:memberId', async (req, res, next) => {
    try {
        ...
        if (req.body.isLead !== undefined)
            data.isLead = req.body.isLead;
        const member = await database_1.prisma.projectTeam.update({
...
router.delete('/:id/team/:memberId', async (req, res, next) => {
    try {
        await database_1.prisma.projectTeam.delete({ where: { id: req.params.memberId } });
        res.json({ success: true });
    }
    catch (err) {
        next(err);
...
router.post('/:id/team/:memberId/notify', async (req, res, next) => {
    try {
        const { items = [], channel = 'notif', message = '' } = req.body;
        if (!Array.isArray(items) || items.length === 0) {
            return res.status(400).json({ success: false, error: 'Aucun élément à envoyer' });
        }
        const member = await database_1.prisma.projectTeam.findUnique({
            where: { id: req.params.memberId },
            include: {
        ...
        const phaseMap = { both: 'installation + démontage', installation: 'installation', dismantling: 'démontage' };
        ...
        if (items.includes('bookings')) {
            const [transports, hotels] = await Promise.all([
                database_1.prisma.teamBooking.findMany({ where: { projectId: p.id, userId: u.id }, orderBy: { onSiteStart: 'asc' } }),
                database_1.prisma.hotelBooking.findMany({ where: { projectId: p.id, occupants: { some: { userId: u.id } } }, orderBy: { checkin: 'asc' } }),
            ]);
            const bkText = [];
        ...
        if (channel === 'notif' || channel === 'both') {
            ...
            try {
                index_1.io.to(`user:${u.id}`).emit('notification:new', { projectId: p.id });
            }
            catch { }
            notifCreated = true;
        }
        ...
        if (channel === 'email' || channel === 'both') {
            ...
            await (0, emailService_1.sendMail)({ to: u.email, subject: `[VEM] ${p.name} — infos projet`, html });
            emailSent = true;
        }
        const bits = [];
        if (notifCreated)
            bits.push('notification envoyée');
        if (emailSent)
            bits.push(`email envoyé à ${u.email}`);
...
function memberEmailShell(title, content) {
...
router.patch('/:id/trucks/:truckId', async (req, res, next) => {
    try {
        const data = {};
        ...
        const truck = await database_1.prisma.truck.update({ where: { id: req.params.truckId }, data });
        ...
        const significantChange = (data.loadingDate !== undefined ||
        ...
        if (significantChange) {
            database_1.prisma.project.findUnique({
                ...
                if (project)
                    (0, telegramService_1.notifyTubizeTruckMovement)(truck, project, 'updated').catch(() => { });
            }).catch(() => { });
        }
...
router.delete('/:id/trucks/:truckId', async (req, res, next) => {
    try {
        await database_1.prisma.truck.delete({ where: { id: req.params.truckId } });
        res.json({ success: true });
    }
    catch (err) {
        next(err);
...
router.get('/:id/trucks', async (req, res, next) => {
    try {
        const trucks = await database_1.prisma.truck.findMany({
...
router.get('/:id/report/pdf', async (req, res, next) => {
    try {
        ...
        (0, saveProjectFile_1.saveProjectFilePdf)(req.params.id, pdf, `Rapport_${report.data.project.internalNumber}.pdf`, 'rapport_projet', req.user?.id);
        res.set({
...
router.get('/:id/files', async (req, res, next) => {
    try {
        const files = await database_1.prisma.projectFile.findMany({
            where: { projectId: req.params.id },
...
router.put('/:id/files/reorder', async (req, res, next) => {
    try {
        const { order } = req.body;
        if (!Array.isArray(order) || !order.length)
            throw new AppError_1.AppError('order (tableau d\'ids) requis', 400);
        await database_1.prisma.$transaction(order.map((fileId, index) => database_1.prisma.projectFile.updateMany({
...
router.post('/:id/files', async (req, res, next) => {
    try {
        ...
        if (!fileUrl)
            throw new AppError_1.AppError('fileUrl requis', 400);
...
router.delete('/:id/files/:fileId', async (req, res, next) => {
    try {
        const file = await database_1.prisma.projectFile.delete({ where: { id: req.params.fileId } });
        // Best-effort — un fichier ajouté par URL externe n'a pas de publicId Cloudinary.
        if (file.publicId)
            await (0, cloudinaryService_1.deleteFromCloudinary)(file.publicId);
        res.json({ success: true });
    }
    catch (err) {
        next(err);
