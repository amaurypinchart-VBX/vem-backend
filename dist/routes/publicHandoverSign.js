...
router.get('/:token', async (req, res, next) => {
    try {
        const h = await database_1.prisma.handover.findFirst({
            where: {
...
router.post('/:token', async (req, res, next) => {
    try {
        ...
        if (h.siteManager?.email)
            recipients.add(h.siteManager.email);
        if (h.project.client?.email)
...
//# sourceMappingURL=publicHandoverSign.js.map