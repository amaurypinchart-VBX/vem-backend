...
router.get('/', async (req, res, next) => {
    try {
        const remarks = await database_1.prisma.clientRemark.findMany({
...
router.get('/:id', async (req, res, next) => {
    try {
        const remark = await database_1.prisma.clientRemark.findUnique({
        ...
        if (!remark)
            return res.status(404).json({ success: false, error: 'Point introuvable' });
...
router.post('/', async (req, res, next) => {
    try {
        const remark = await database_1.prisma.clientRemark.create({
...
router.patch('/:id', async (req, res, next) => {
    try {
        ...
        const remark = await database_1.prisma.clientRemark.update({ where: { id: req.params.id }, data });
        res.json({ success: true, data: remark });
    }
    catch (err) {
        next(err);
...
router.delete('/:id', async (req, res, next) => {
    try {
        await database_1.prisma.clientRemark.delete({ where: { id: req.params.id } });
...
router.post('/:id/photos', cloudinaryService_1.upload.single('file'), async (req, res, next) => {
    try {
        if (!req.file)
            return res.status(400).json({ success: false, error: 'Fichier manquant' });
        const { url, publicId } = await (0, cloudinaryService_1.uploadToCloudinary)(req.file.buffer, 'remarks');
        const photo = await database_1.prisma.clientRemarkPhoto.create({
...
router.get('/project/:projectId/pdf', async (req, res, next) => {
    try {
        const remarks = await database_1.prisma.clientRemark.findMany({
        ...
        await new Promise((resolve) => {
            ...
            for (const r of remarks) {
                ...
                if (r.zone)
                    doc.fillColor('#8892a4').font('Helvetica').fontSize(9).text(`📍 ${r.zone}`, 58, y - 24);
                if (r.description)
                    doc.fillColor('#8892a4').fontSize(9).text(r.description, 58, y - 14, { width: 440 });
                if (r.assignedToUser)
                    doc.fillColor(color).fontSize(9).text(`👤 ${r.assignedToUser.firstName} ${r.assignedToUser.lastName}`, 410, y - 38, { width: 140, align: 'right' });
...
//# sourceMappingURL=clientRemarks.js.map